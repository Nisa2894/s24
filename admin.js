    ...
    async function loadSeekers() {
        try {
            ...
            seekers.forEach(seeker => {
                ...
                card.className = 'bg-white p-6 rounded-lg shadow-lg';
                card.innerHTML = `
                    <h3 class="text-xl font-semibold">${seeker.name}</h3>
    ...
    window.toggleProviderVerification = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                alert('Error updating provider verification: ' + result.error);
            }
        } catch (error) {
            console.error(error);
    ...
    window.toggleSeekerVerification = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                const result = await response.json();
                alert('Error updating seeker verification: ' + result.error);
            }
        } catch (error) {
            console.error(error);
            alert('Error: ' + error.message);
